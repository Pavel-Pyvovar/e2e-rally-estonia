...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, channels: int, n_heads: int, n_layers: int, d_cond: int):
        ...
        # self.norm = torch.nn.GroupNorm(num_groups=4, num_channels=channels, eps=1e-6, affine=True)
...
class PilotAtt(nn.Module):
    ...
    def __init__(self, 
        ...
        self.att1 = SpatialTransformer(
            24, n_heads, n_att_layers, 3)
        ...
        self.att2 = SpatialTransformer(
            36, n_heads, n_att_layers, 3)
        ...
        self.att3 = SpatialTransformer(
            48, n_heads, n_att_layers, 3)
        ...
        self.att4 = SpatialTransformer(
            64, n_heads, n_att_layers, 3)
    ...
    def forward(self, x, control):
        x = self.block1(x)
        x = self.att1(x, cond=control)
        x = self.block2(x)
        x = self.att2(x, cond=control)
        x = self.block3(x)
        x = self.att3(x, cond=control)
        x = self.block4(x)
        x = self.att4(x, cond=control)
        x = self.block5(x)
        x = self.regressor(x)
        return x
